...
func renderRail(width, height int, w model.Worktree, radar *radarView) string {
	...
	fmt.Fprintln(&b)
	fmt.Fprintln(&b)
	fmt.Fprint(&b, renderApproveFooter(w.Base, reviewed, total))
...
func renderRailRow(width int, f model.DiffFile, reviewed, current bool) string {
	...
	row := clipWidth(fmt.Sprintf(" %s %s %s", box, nameStyle.Render(name), stats), width)
	if current {
...
// renderApproveFooter is the mock's progress bar + "✓ Approve & merge to
// <base>" affordance ("(k left)" when incomplete) — P3-design.md §1.1.
// Approve itself is driven by the `a` key elsewhere; this is a status
// indicator, not a widget.
func renderApproveFooter(base string, reviewed, total int) string {
	label := fmt.Sprintf("Reviewed %d / %d", reviewed, total)
	bar := progressBar(reviewed, total, 16)
	approve := fmt.Sprintf("✓ Approve & merge to %s", base)
	if reviewed < total {
		approve += fmt.Sprintf(" (%d left)", total-reviewed)
	}
	return label + "\n" + bar + "\n" + styles.Accent.Render(approve)
}
